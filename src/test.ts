...
provider.on("pending", async(txHash) => {
    ...
    if (txResponse.to === "0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D") {
        const decodedInput = iface.parseTransaction({ data: txResponse.data, value: txResponse.value});
        let txInfo: any;
        switch (decodedInput.name) {
            case "swapExactTokensForTokens":
                txInfo = {
                    ...
                    params: {
                        amountIn: decodedInput.args[0].toString(),
                        amountOutMin: decodedInput.args[1].toString(),
                        path: decodedInput.args[2],
                        to: decodedInput.args[3],
                        deadline: decodedInput.args[4].toString()
                    }
                }
                break;
            case "swapTokensForExactTokens":
                txInfo = {
                    name: decodedInput.name,
                    params: {
                        amountOut: decodedInput.args[0].toString(),
                        amountInMax: decodedInput.args[1].toString(),
                        path: decodedInput.args[2],
                        to: decodedInput.args[3],
                        deadline: decodedInput.args[4].toString()
                    }
            ...
            case "swapExactETHForTokens":
                txInfo = {
                    name: decodedInput.name,
                    params: {
                        amountOutMin: decodedInput.args[0].toString(),
                        path: decodedInput.args[1],
                        to: decodedInput.args[2],
                        deadline: decodedInput.args[3].toString()
                    }
                }
                break;
